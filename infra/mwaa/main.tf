...
resource "aws_iam_role_policy" "mwaa" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    ...
    { Effect = "Allow", Action = "secretsmanager:GetSecretValue", Resource = "arn:aws:secretsmanager:${local.config.region}:${local.config.account}:secret:${local.project}/airflow/variables/*" },
...
resource "aws_mwaa_environment" "workshop" {
  ...
  airflow_configuration_options = {
    ...
    "secrets.backend_kwargs" = jsonencode({
      variables_prefix   = "${local.project}/airflow/variables"
      connections_prefix = null
      config_prefix      = null
    })
  }
